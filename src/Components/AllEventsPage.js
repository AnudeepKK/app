...
import EventCarousel from './YourComponent';
import "./allevent.css"
...
const AllEventsPage = () => {
  ...
  const displayedEvents = (selectedCategory !== 'all' ? eventData.filter(event => event.type === selectedCategory) : eventData)
      ...
      <Container fluid>
        <Row>
          ...
          <Col xs={12} sm={6} md={4} lg={4} xl={6}>
            <div className={`sub-navbar d-flex flex-column  align-items-center py-3 m-3 ${selectedCategory ? 'glow-border' : ''}`} onKeyDown={handleKeyDown} role="navigation" tabIndex="0">
              <ul className="nav flex-column text-center">
                {Object.keys(categoryHeadings).map((category) => (
                  <li className="nav-item" key={category}>
                    <button
                      className={`nav-link category-button btn btn-link text-light ${selectedCategory === category ? 'active' : ''}`}
          ...
          {/* Search bar */}
          <Col xs={12} sm={6} md={8} lg={8} xl={6} className='right-container glow-border'>
            <div className="d-flex flex-column align-items-center  py-3 m-3 search-bar-container ">
              ...
              {/* Day navigation */}
              <Col xs={12} sm={6} md={4} lg={4} xl={6}>
                <div className={`d-flex flex-column  align-items-center py-3 m-3 ${selectedDay ? 'glow-border' : ''}`} onKeyDown={handleKeyDown} role="navigation" tabIndex="0">
                  <ul className="nav flex-column text-center">
      ...
      <div className='heading-class'>
        <h1 className="text-center m-1 text-info">
          {showResults ? (
            ...
            selectedDay === '1' ? "Day 1 Events" : selectedDay === '2' ? "Day 2 Events" : categoryHeadings[selectedCategory]
          )}
        </h1>
      </div>
  ...
  {(showResults ? searchResults : displayedEvents).map((event, index) => (
    <div
      key={index}
      className="col-lg-2 col-md-3 col-sm-6 col-12 mx-5 rounded my-3 text-center event-card glow-border"
    
    >
      <div className="card text-light bg-info p-2 h-100">
        <img
          src={event.image}
          alt="Event"
        ...
        <div className="card-body bg-info">
          ...
          <p className="card-text">{event.date} at {event.time}</p>
          <button className="btn btn-primary" onClick={() => openModal(event)}>Details</button>
